...
if st.button("🔍 Run Punching Shear Check"):
    ...
    if vu > Oo * Vc:
        ...
        if Vu <= Oo * Vc_max:
            st.warning("⚠️ However, it can be reinforced with shear reinforcement.")

            with st.expander("🧮 Shear Reinforcement Design"):
                fy = st.number_input("Yield strength of shear reinforcement fy (MPa)", min_value=0.0, step=50.0)
